## Mi perfil👋

# ¡Hola 👋! Soy Juan Diego Mora Cabrales  
**Estudiante de Ingeniería de Sistemas | Desarrollador | QA | IoT**

---

## 🚀 Sobre mí

Soy estudiante de **Ingeniería de Sistemas** con materias finalizadas y en proceso de titulación. Me apasiona el desarrollo de software, el aseguramiento de la calidad (QA) y la integración de tecnologías como IoT y cloud.  
Cuento con experiencia en:

- Desarrollo con **Python, Java, C#, JavaScript, Angular, Kotlin**
- Pruebas de software: **smoke tests, unitarias, regresión, caja negra**
- Diseño y ejecución de planes de prueba y reporte de bugs
- Bases de datos **SQL y NoSQL**
- Simulación de redes con **Cisco Packet Tracer**
- Automatización de tareas con **Python**

## 🛠️ Tecnologías y Herramientas

| Área           | Tecnologías                                                                 |
|----------------|-----------------------------------------------------------------------------|
| **Lenguajes**  | Python, Java, C#, JavaScript, Kotlin, SQL                                   |
| **Frontend**   | HTML, CSS, Angular                                                          |
| **QA**         | Planes de prueba, casos de uso, reporte de bugs, pruebas funcionales        |
| **Bases de Datos** | MySQL, MongoDB (SQL/NoSQL)                                              |
| **Redes & Cloud** | Cisco Packet Tracer, Arduino Cloud, IoT                                  |
| **Herramientas** | Git, GitHub, Coursera, Udemy, Cisco NetAcad                                 |

## 📁 Experiencia Destacada

### 💻 Prácticas Profesionales – Tecnoparque Nodo Ocaña (2025)
- Diseño y ejecución de planes de prueba para software.
- Reporte y seguimiento de bugs.
- Desarrollo de un **sistema de monitoreo IoT** para cultivo hidropónico con Arduino Cloud, sensores y dashboards en tiempo real.

## 📚 Educación y Cursos

- **Ingeniería de Sistemas** – Universidad Francisco de Paula Santander, Ocaña *(En curso – 10mo semestre)*  
- **Bootcamp en Análisis de Datos** – Talento Tech Oriente (2025)  
- **Automatiza tareas de ciberseguridad con Python** – Coursera (2024)  
- **Introducción a Ciberseguridad** – Cisco Networking Academy (2024)  
- **Mega Curso de Python: 10 Aplicaciones Reales** – Udemy (2025)  

## 📊 Estadísticas de GitHub

<div align="center">
  
[![GitHub Streak](https://streak-stats.demolab.com/?user=JuanMora11&theme=dark)](https://git.io/streak-stats)

[![Estadísticas de GitHub](https://github-readme-stats.vercel.app/api?username=JuanMora11&show_icons=true&theme=radical)](https://github.com/anuraghazra/github-readme-stats)

[![Lenguajes más usados](https://github-readme-stats.vercel.app/api/top-langs/?username=JuanMora11&layout=compact&theme=vision-friendly-dark)](https://github.com/anuraghazra/github-readme-stats)

## 🌟 Habilidades Blandas

- Rápido aprendizaje  
- Trabajo en equipo  
- Adaptabilidad  
- Resolución de problemas  
- Comunicación efectiva  
- Liderazgo  

## 📫 Contáctame

- 📧 Email: juan.0909.123@gmail.com  
- 💼 LinkedIn: https://www.linkedin.com/in/juan-diego-mora-cabrales-992359290/
- 🌐 GitHub: [TuUsuarioGitHub](https://github.com/TuUsuarioGitHub)

---
